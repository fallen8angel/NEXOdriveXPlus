...
def test_reverse_driver_camera_setting_is_removed(settings, params):
  by_name = {p["name"]: p for p in params}
  assert "ReverseDriverCamera" not in by_name

  vehicle = next(category for category in settings["menu"] if category["id"] == "VEHICLE")
  driver_monitor = next(group for group in vehicle["groups"] if group["id"] == "VEH_DM")
  assert "ReverseDriverCamera" not in driver_monitor["params"]

  params_keys = PARAMS_KEYS_PATH.read_text(encoding="utf-8")
  assert "ReverseDriverCamera" not in params_keys
