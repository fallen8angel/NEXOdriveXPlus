...
class ClusterUiRenderer:
    ...
    def _render_world(self, state: ClusterUiState, signal_lights: tuple[bool, bool] | None = None) -> None:
        self._ego_brake_lit = bool(state.brake > 0.001)
        if signal_lights is None:
            signal_lights = self._turn_signal_lights(state)
    ...
    def _draw_vehicle(self, vehicle: VehicleBox) -> None:
        ...
        if use_model:
            ...
            if not vehicle.source and getattr(self, "_ego_brake_lit", False):
    ...
    def _draw_vehicle_brake_lights(self, vehicle: VehicleBox) -> None:
        """Overlay left/right ego stop lamps without changing the base vehicle model."""
        rear_offset = -vehicle.length_m * 0.501
        light_bottom = 0.035 + vehicle.height_m * 0.735
        light_top = 0.035 + vehicle.height_m * 0.775
        ...
        inner = vehicle.width_m * 0.24
        for x0, x1 in ((-outer, -inner), (inner, outer)):
            self._draw_quad(
                rear_point(x0, light_bottom),
                rear_point(x1, light_bottom),
                rear_point(x1, light_top),
                rear_point(x0, light_top),
                (255, 28, 20, 255),
            )
